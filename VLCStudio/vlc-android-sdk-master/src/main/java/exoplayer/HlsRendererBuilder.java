...
public class HlsRendererBuilder implements RendererBuilder {
  ...
  @Override
  public void buildRenderers(exoplayer.DemoPlayer player) {
    //播放器prepare的时候调用该方法，从而执行创建渲染器
  ...
  @Override
  public void cancel() {
    //在buildRenderers前调用，释放资源的时候同样调用
  ...
  private static final class AsyncRendererBuilder implements ManifestCallback<HlsPlaylist> {
    ...
    private final String userAgent;
    private final String url;
    private final exoplayer.DemoPlayer player;
    private final ManifestFetcher<HlsPlaylist> playlistFetcher;

    private boolean canceled;

    public AsyncRendererBuilder(Context context, String userAgent, String url, exoplayer.DemoPlayer player) {
      this.context = context;
      this.userAgent = userAgent;
      this.url = url;
      this.player = player;
    ...
    public void onSingleManifest(HlsPlaylist manifest) {
      ...
      HlsSampleSource sampleSource = new HlsSampleSource(chunkSource, loadControl,
          MAIN_BUFFER_SEGMENTS * BUFFER_SEGMENT_SIZE, mainHandler, player, exoplayer.DemoPlayer.TYPE_VIDEO);
      ...
      if (preferWebvtt) {
        ...
        HlsSampleSource textSampleSource = new HlsSampleSource(textChunkSource, loadControl,
            TEXT_BUFFER_SEGMENTS * BUFFER_SEGMENT_SIZE, mainHandler, player, exoplayer.DemoPlayer.TYPE_TEXT);
        textRenderer = new TextTrackRenderer(textSampleSource, player, mainHandler.getLooper());
      } else {
        textRenderer = new Eia608TrackRenderer(sampleSource, player, mainHandler.getLooper());
      }

      TrackRenderer[] renderers = new TrackRenderer[exoplayer.DemoPlayer.RENDERER_COUNT];
      renderers[exoplayer.DemoPlayer.TYPE_VIDEO] = videoRenderer;
      renderers[exoplayer.DemoPlayer.TYPE_AUDIO] = audioRenderer;
      renderers[exoplayer.DemoPlayer.TYPE_METADATA] = id3Renderer;
